...
void FlyingEnemy::update(sf::Time delta)
{
    ...
    if (m_sprite.getPosition().x < 0 || m_sprite.getPosition().y + m_sprite.getGlobalBounds().height > SCREEN_Y_SIZE
        || m_sprite.getPosition().y < 0 || m_sprite.getPosition().x + m_sprite.getGlobalBounds().width > SCREEN_X_SIZE)
